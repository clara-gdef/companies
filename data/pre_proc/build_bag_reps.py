...
def build_and_save_rep_for_bag(bag_type, bag_dict, ppl_dict, agg_name, agg_func):
    ...
    for bag_num, bag_name in tqdm(bag_dict.items(),
                                  desc="Building rep of " + bag_type.upper() + " for agg " + agg_name.upper() + "..."):
        ppl_ft_emb = [ppl_dict[i]["ft"] for i in ppl_dict if ppl_dict[i][bag_type + "_name"] == bag_name]
        ppl_skills = [ppl_dict[i]["sk"] for i in ppl_dict if ppl_dict[i][bag_type + "_name"] == bag_name]
    ...
    with open(os.path.join(cfg["datadir"], bag_type + "_rep_" + agg_name + ".pkl"), "wb") as f:
        pkl.dump(bag_reps, f)
